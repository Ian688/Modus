...
```
Wave0 并发重活架构层（2-3 周）──► Wave1 韧性地基 ──► Wave3 A1/A2 信任审批 ──► Wave2 C1 缓存 ──► Wave1 T4 多实例
   │                                    │                    │                        │
   │                             └──► Wave2 C2/C3 ──► Wave4 G1/G2 自主性 ──► Wave3 A3 coverage ──► Wave5 E1 评估 ──► 其余排期
```

**为什么这个顺序**：**先 Wave 0**（搭"同时干重活 + 内存不抱怨"的并发调度层，也是未来 Rust 下沉的地基）→ 再稳地基（T1/T2/T3，其中 T1 进程出身与 Wave0 同动进程层）→ 再补审批短板（A1/A2 成本最低）→ 再拿缓存红利（C1）→ 再改核心循环（G1 战略价值最高但要地基稳）→ 评估/进化远期。

**Wave 0 是本次讨论的增量**：用户视角要求"同时干重活 + 内存回收不抱怨"。答案不是换语言，而是"Python 核心 + 多进程 worker 隔离 + 内存有界回收"，worker 边界 = MCP 协议，未来 Rust 下沉只换实现不动边界。

## 五波文档

| 文档 | 内容 | 子项 | 工期 | 来源 |
|---|---|---|---|---|
| [dev-wave0-concurrency.md](dev-wave0-concurrency.md) | **并发重活架构层**（用户视角增量） | W0-1 多进程 worker 隔离+内存上限<br>W0-2 office_exec 下沉 worker<br>W0-3 worker 事件订阅+Tick 节拍<br>W0-4 未来 Rust 下沉边界契约 | 2-3 周 | supervisor+hermes+loop+beads |
